...
class TextSystem(object):
    def __init__(self, args):
        ...
        self.yolov5_detector = torch.hub.load('./', 'custom', path='./exp23/weights/best.pt',
    ...
    def __call__(self, image, cls=True):
        ...
        for item in pred:
            ...
            cls = round(item[5])
            lovo = [cls, x / 640, y / 640, w / 640, h / 640]
...
class ocrClass:
    ...
    def __init__(self) -> None:
        ...
        # self.crop_save_dir = 'D:/work/jianhua_data_origin/2022_11/2_2/detected'
        self.crop_save_dir = imgPath + '/detected'

        print('ocrClass __init__ test')

    def draw_boxes(self, frame, dt_boxes):
        # dt_boxes = np.array(dt_boxes).astype(int)
    ...
    def save_crop(self, file_name, crop_list):
        print('save crop:', file_name)
        file_name = file_name.split('.')[0]
        code_plate_crop_dir = os.path.join(self.crop_save_dir, 'm')
        manual_code_crop_dir = os.path.join(self.crop_save_dir, 'd')
        # print(code_plate_crop_dir)
        if not os.path.exists(code_plate_crop_dir):
            print('makedir:', code_plate_crop_dir)
            os.makedirs(code_plate_crop_dir)
        if not os.path.exists(manual_code_crop_dir):
            os.makedirs(manual_code_crop_dir)

        code_plate_i, manual_code_i = 0, 0

        for img, cls in crop_list:
            if cls == 0:
                save_name = os.path.join(code_plate_crop_dir, '{}_{}_{}.jpg'.format(file_name, 'm', code_plate_i))
                code_plate_i += 1
            elif cls == 1:
                save_name = os.path.join(manual_code_crop_dir, '{}_{}_{}.jpg'.format(file_name, 'd', manual_code_i))
                manual_code_i += 1
            cv2.imwrite(save_name, img)

    def detect(self, img, file_name, save_crop=False):
        dt_boxes, dt_boxes_crop_list = self.text_sys(img, cls=False)

        if (len(dt_boxes) == 0):
            print('--ocr failed')
            cv2.putText(img, '--ocr failed', (30, 50), cv2.FONT_HERSHEY_COMPLEX, 0.6, (255, 0, 0), 1)
            return img, 0

        if save_crop:
            self.save_crop(file_name, dt_boxes_crop_list)

        return self.draw_boxes(img, dt_boxes), len(dt_boxes)


if __name__ == "__main__":
    imgPath = 'E:\\yolov5-master\\datasets\\Contain\\'
    ...
    for f in images:
        ...
        if (post_name == 'jpg'):
            ...
            res_img, box_num = model.detect(img, file_name, save_crop=True)
            win_name = 'OCR: ' + f
            # cv2.namedWindow(win_name, cv2.WINDOW_NORMAL)
            # cv2.imshow('ocr', res_img)
            save_name = imgPath + '/detected/' + file_name + '_ocr.' + post_name
            save_name = imgPath + '/detected/' + file_name + '.' + post_name
